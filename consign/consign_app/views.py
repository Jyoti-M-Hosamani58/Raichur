...
from consign_app.models import Login,Collection,LiveLocation,CollectionBalance, AddConsignment,AddConsignmentTemp,Disel, AddTrack,FeedBack, Branch,Driver,Vehicle, Staff,Consignee, Consignor,TripSheetTemp,TripSheetPrem, Account,Expenses
...
def addConsignment(request):
    if request.method == "POST":
        ...
        # Initialize amount and balance based on pay_status
        if pay_status == 'Paid':
            # If the payment status is Paid, use the entered amount
            amount = float(amount)
            balance = cost - amount
        if pay_status == 'ToPay':
            # If the payment status is ToPay, set amount to 0 and balance to the full cost
            amount = 0
            balance = cost
        ...
        # Only handle the Collection model if pay_status is 'Paid' or 'ToPay'
        if pay_status in ['Paid', 'ToPay']:
            # Assign sender_name based on pay_status
            if pay_status == 'Paid':
                sender_name = send_name
            elif pay_status == 'ToPay':
                sender_name = rec_name

            # Create the Collection object with the assigned sender_name
            Collection.objects.create(
                sender_name=sender_name,  # Use the conditionally assigned sender_name
                date=now,
                lrNo=con_id,
            ...
            if pay_status in ['Paid', 'ToPay']:
                ...
                # Create the Collection object with the assigned sender_name
                CollectionBalance.objects.create(
                    sender_name=sender_name,  # Use the conditionally assigned sender_name
                    date=now,
                    pay_status=pay_status,
                    total=cost,
...
def branchConsignment(request):
    if request.method == "POST":
        ...
        qr_image_file = None  # Initialize qr_image_file with None
        payment_amount = int(cost * 100)  # Razorpay accepts amount in paise
        qr_code_number = str(con_id)  # Use Consignment ID as the QR code number

        try:
            # Create a QR code via Razorpay
            qr_response = razorpay_client.qrcode.create({
                "type": "upi_qr",  # Type of QR code
                "name": f"Payment for Consignment {con_id}",
                "usage": "single_use",  # Single-use QR code
                "fixed_amount": True,
                "payment_amount": payment_amount,  # Amount in paise
                "description": f"Payment for consignment {con_id}",
                "customer_name": send_name,
                "customer_email": "customer@example.com",  # Replace with actual customer email
                "customer_contact": send_mobile,  # Customer's mobile number
            })

            # Retrieve the QR code URL from the response
            qr_code_url = qr_response['image_url']

            # Download the QR code image
            qr_code_image_response = requests.get(qr_code_url)
            qr_code_image = Image.open(BytesIO(qr_code_image_response.content))

            # Save QR code image to your Django model
            qr_image_io = BytesIO()
            qr_code_image.save(qr_image_io, format='PNG')
            qr_image_file = ContentFile(qr_image_io.getvalue(), f'{qr_code_number}.png')
        ...
        # Only handle the Collection model if pay_status is 'Paid' or 'ToPay'
        if pay_status in ['Paid', 'ToPay']:
            sender_name = send_name if pay_status == 'Paid' else rec_name
            Collection.objects.create(
        ...
        for product, piece in zip(products, pieces):
            ...
            AddConsignment.objects.create(
                ...
                qrcode=qr_image_file,  # Will be None if QR code generation failed
            )
            AddConsignmentTemp.objects.create(
                track_id=con_id,
                Consignment_id=Consignment_id,
        ...
        messages.success(request, f'Consignment {con_id} added successfully')
...
def get_balance(request):
    ...
    if sender_name:
        try:
            # Fetch the most recent balance for the sender
            collection = CollectionBalance.objects.filter(sender_name=sender_name).order_by('-date').first()

            if collection:  # If a collection record is found
                return JsonResponse({'balance': str(collection.balance)})  # Return balance as a string
            else:  # No records found for the given sender_name
                return JsonResponse({'balance': None})
        except Exception as e:  # Catch any unexpected exceptions
            return JsonResponse({'balance': None, 'error': str(e)})
    return JsonResponse({'balance': None})  # Return None if sender_name is not provided
...
def consignmentStatus(request):
    ...
    if request.method == 'POST':
        ...
        # Start building the query
        queryset = AddConsignment.objects.filter(consignment_status='Pending')  # Fetch only Pending consignments
...
def complete_consignment(request, track_id):
    ...
    return redirect('consignmentStatus')  # Replace 'consignment_list' with the name of your success page or list page
...
@csrf_exempt  # For simplicity in this example; use proper CSRF tokens in production
def update_location(request):
    if request.method == 'POST':
        ...
        # Here, we assume the user is authenticated and we can get their user object
        user = request.user  # Or you can fetch the user some other way (e.g., from the session)

        # Save or update the LiveLocation in the database
        LiveLocation.objects.create(user=user, latitude=latitude, longitude=longitude)
